...
const cardsLoader = new DataLoader(async (keys) => {
  const keysList = keys.join(', ');
  try {
    const data = await executeQuery(`SELECT * FROM cards WHERE object_id IN (${keysList}) ORDER BY array_position(ARRAY[${keysList}], object_id)`);
    const cardsById = groupBy(data.rows, 'object_id');
    return keys.map(id => cardsById[id]);
  } catch (err) {
    console.log(err);
    return null;
  }
